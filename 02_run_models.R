...
S1 <- data$GEOID # for city-level spatial random effect
...
baseformula3 <- Y ~ f(T1, replicate = S1, model = "rw1", cyclic = TRUE, constr = TRUE,
                      scale.model = TRUE,  hyper = precision.prior2) +
  f(S1, model = "bym2", replicate = T2, graph = "texas/map.graph",
    scale.model = TRUE, hyper = precision.prior2)  
...
models <- lapply(1:length(formulas),
                 function(i) {
                   ...
                   save(model, file = paste0("texas/", lab[i],".RData"))})

# create table to store DIC and select best model
table1 <- data.table(Model  =  c("base3","base4"),
                     DIC = NA,
                     logscore = NA)

for(i in 1:length(formulas))
{
  load(paste0("texas/",lab[i],".RData"))
...
fwrite(table1, file = "texas/best_model_selection0.csv", quote = FALSE,
       row.names = FALSE)
...
# Write results of model selection
fwrite(table1, file = "texas/best_model_selection1.csv", quote = FALSE,
       row.names = FALSE)


# define formulas by updating the baseline formula with different combinations of 
#cross-basis functions of temperature, precipitation, covid cases, and best-fit intervention index
